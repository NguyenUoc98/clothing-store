    ...
    <div class="w-full rounded-xl bg-white shadow p-6 mb-10">
        <form action="{{ route('reports.revenue') }}" method="get" class="flex items-end gap-6">
            ...
            <div class="space-y-2 flex flex-col">
                ...
                <input type="date" name="end_date" id="end_date" class="p-2 border rounded-lg" value="{{ request()->input('start_date', Carbon\Carbon::now()->toDateString()) }}">
            </div>
        ...
        <table class="table">
            ...
            @forelse ($statistics as $statistic)
                <tr>
                    ...
                    <td class="whitespace-nowrap !text-nowrap font-bold text-red-600 text-center"><span>{{ number_format($statistic->money, 0, ',', '.') }} VND</span></td>
                </tr>
            @empty
                <tr>
