...
use Uocnv\MomoPayment\MomoPayment;
...
class CheckoutController extends Controller
{
    ...
    public function resultMomo(Request $request)
    {
        ...
        if ($errorCode == 0) {
            $orderProcessed = Order::query()
                ->where('status', PaymentStatus::SUCCESS)
                ->whereJsonContains('addition_information->transaction_id', $transId)
                ->exists();
            if ($orderProcessed) {
                $dataReturn = [
                    'resultCode' => 100,
        ...
        try {
            ...
            if ($paymentMethod == PaymentType::ONLINE) {
                try {
                    ...
                    $response    = $momoPayment->createRequest([
                        'orderId'   => $orderId,
                        'amount'    => $order->total_price,
                        'requestId' => rand(1, 99).time(),
                        'returnUrl' => route('order.confirmation', ['orderId' => $order->id]),
